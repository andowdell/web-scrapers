...
MAIN_URL = "https://bordeauxindex.com/livetrade/marketplace"
PAGE_URL = "https://bordeauxindex.com/livetrade/next-page"
...
class Extractor:
    ...
    def start(self):
        ...
        token = resp.headers['set-cookie'].split(';')[0]
        print(token[11:-3])
        login_headers = {
            'X-Xsrf-Token':token[11:-3],
            'x-requested-with': 'XMLHttpRequest',
            'Content-Type': 'application/x-www-form-urlencoded', # its urlencoded instead of form-data
            'User-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/81.0.4044.129 Safari/537.36',
        }
        resp = self.session.post(PAGE_URL, headers=login_headers, data={
            'category': "",
            'colour': "",
            'packsize': "",
            'page': 2,
